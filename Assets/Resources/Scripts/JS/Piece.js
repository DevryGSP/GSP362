...
function Start  ()
{
	...
	fastDrop = GameObject.Find("QuickDrop").GetComponent("Button");
	rotPiece = GameObject.Find("RotButton").GetComponent("Button");
	
	fastDrop.onClick.AddListener(FastDrop);
	rotPiece.onClick.AddListener(RotClock);
	// add ghost piece
...
function Update ()
{
	...
	{
		if (canMovePiece && (Input.GetAxis("Vertical") < 0 || Input.GetAxis("GP_Vertical") < 0 || joystick.position.y > .1f))
		{
			canMovePiece = false;
			if (!Collide(this.transform.position.x, this.transform.position.y + 1f))
			{
				this.transform.position.y += 1f;
				UpdateGhost();
			}
		}
		else if (canMovePiece && (Input.GetAxis("Vertical") > 0 || Input.GetAxis("GP_Vertical") > 0 || joystick.position.y < -.1f))
		{
			canMovePiece = false;
			if (!Collide(this.transform.position.x, this.transform.position.y - 1f))
			{
	...
	{
		if (canMovePiece && (Input.GetAxis("Horizontal") < 0 || Input.GetAxis("GP_Horizontal") < 0 || joystick.position.x < -.1f))
		{
			canMovePiece = false;
			if (!Collide(this.transform.position.x - 1f, this.transform.position.y))
			{
				this.transform.position.x -= 1f;
				UpdateGhost();
			}
		}
		else if (canMovePiece && (Input.GetAxis("Horizontal") > 0 ||  Input.GetAxis("GP_Horizontal") > 0 || joystick.position.x > .1f))
		{
		 	canMovePiece = false;
			if (!Collide(this.transform.position.x + 1f, this.transform.position.y))
			{
